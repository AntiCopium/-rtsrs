...
import { ApplicationCommandOptionTypes, ApplicationCommandTypes, InteractionResponseTypes, validatePermissions } from "../../deps.ts";
  ...
  execute: async (Bot, interaction) => {
    ...
    if (interaction.data?.options[0].value! === 'enable') {
      ...
      await Bot.helpers.sendInteractionResponse(
        ...
        UserConfigSettings.set(UserConfigOptions.MessageDeletionLogSetting, false);
      await Bot.helpers.sendInteractionResponse(
        interaction.id,
        interaction.token,
        {
